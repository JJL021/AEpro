...
module uart_tx(
    ...
    (*mark_debug = "true"*)input               uart_tx_en_in  , //UART的发送使能
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        uart_tx_en_d1 <= uart_tx_en_in;
        uart_tx_en_d2 <= uart_tx_en_d1;
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) 
        baud_cnt <= 16'd0;
    else if(uart_tx_en)     //上一个数据已经发送完毕了，只是还有个停止位没发完。不到一个bit周期就提前结束了
        baud_cnt <= 16'd0;      
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n)  begin
        ...
        else if(block_cnt == 9'd255) begin
            if(block_cnt2 < 9'd255)
                uart_tx_data <= block_cnt2;
            else if(block_cnt2 == 9'd255)
                uart_tx_data <= block_cnt3;
            else
                uart_tx_data <= block_cnt2;
        end
        else
...
always @(posedge clk or negedge rst_n) begin
    ...
    else if (block_cnt3 == 2'd3 && block_cnt2 == 8'd255 &&  block_cnt2 == 8'd255) begin
        flag_192000 <= 1'b1;
    end
end
