...
// localparam my_st_idle    = 1'b0;
// localparam my_st_2    = 1'b1;
...
(*mark_debug = "true"*)reg          trig_tx_en     ;
...
(*mark_debug = "true"*)reg          my_start_en_d1    ; //block_flag打拍寄存器
                                    
//wire define                       
(*mark_debug = "true"*)wire         pos_start_en    ;//开始发送数据上升沿
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        my_start_en_d0 <= block_flag;
        my_start_en_d1 <= my_start_en_d0;
    end
end 
...
// always @(posedge clk or negedge rst_n) begin
//     if(!rst_n)
//         my_cur_state = my_st_idle;
//     else
//         my_cur_state <= my_next_state;
// end

// always @(*) begin
//     my_next_state = my_st_idle;
//     case(my_cur_state)
//         my_st_idle :       begin
//             if(my_skip_en)
//                 my_next_state <= my_st_2;
//             else
//                 my_next_state <= my_st_idle;
//         end
//         my_st_2 :      begin
...
//         case(my_next_state)
//             my_st_idle  : begin
//                 if(sustain_flag == 1'b1)
//                     my_skip_en <= 1'b1;
//                 else;
//             end
//             my_st_2   : begin
//                 if(block_flag == 1'b1) begin
//                     my_skip_en <= 1'b1;
//                     block_flag_t <= 1'b1;
//                 end
//                 else begin
...
always @(posedge clk or negedge rst_n) begin
    if(!rst_n) begin
        ...
        case(next_state)
            st_idle     : begin
                if(trig_tx_en || my_start_en_d1) begin   //接受完一包数据或写入256B字节时启动发送
                    skip_en <= 1'b1; 
