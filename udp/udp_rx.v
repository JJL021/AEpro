...
module udp_rx(
    ...
    output  reg          start_collect_flag  //AD开始采集标志位
    );
...
localparam  start_collect_cmd = 8'haa;
localparam  stop_collect_cmd  = 8'hbb;
            ...
            st_rx_data : begin         
                ...
                if(gmii_rx_dv) begin
                    ...
                    if(data_cnt == data_byte_num - 16'd1) begin
                        ...
                        data_cnt <= 16'd0;
                        rec_pkt_done <= 1'b1; 
                        if(rec_data == start_collect_cmd) begin //如果收到开始数据采集命令
                            start_collect_flag <= 1'b1;
                            sustain_flag <= 1;
                        end
                        else if(rec_data == stop_collect_cmd) begin //如果收到停止数据采集命令
                            start_collect_flag <= 1'b0;
                            sustain_flag <= 0;      
                        end                     
                        else begin
